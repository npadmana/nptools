...
void GSLHist::setbins(double xmin, double xmax, int n) {
    ...
    gsl_histogram_set_ranges_uniform(whist.get(), xmin, xmax);
}

void GSLHist::setbins(const VectorXd& bins) {
    alloc(bins.size());
    gsl_histogram_set_ranges(chist.get(), bins.data(),  bins.size());
    gsl_histogram_set_ranges(whist.get(), bins.data(),  bins.size());
}
...
MatrixXd GSLHist::val() {
   ...
   for (int ii=0; ii< nbins; ++ii) {
       ...
       gsl_histogram_get_range(whist.get(), ii, &rmin, &rmax);
       retval(ii, 0) = rmin;
       retval(ii, 1) = rmax;
   }
   return retval;
